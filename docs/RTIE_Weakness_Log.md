...
## W148. Explainer fabricates December calendar gating on FN_G_TEST_CSTM — FIXED 2026-06-01

**Status:** Implemented and merged on `fix/w148-calendar-fabrication`. Prompt-layer fix in `SEMANTIC_EXPLANATION_PROMPT` ([src/agents/logic_explainer.py](src/agents/logic_explainer.py)). Originally logged 2026-06-01 during W147 close-out. **NOT caused by W147** — see "Exposure, not regression" below.

**Root cause (generation-side, confirmed).** `SEMANTIC_EXPLANATION_PROMPT` Rule 4 hardcoded the December gate as the *worked exemplar* of an execution condition — the literal string `"This entire function ONLY runs when the reporting month is December."` — under an unconditional *"Always include execution conditions prominently"* directive, with Rule 7 echoing `(e.g. December-only)`. The prompt taught the model the exact phrase W57 Check 5 then flags. A decisive isolated probe (drive `stream_semantic` on FN_G_TEST_CSTM's clean body alone, empty description, no sibling functions) reproduced the December gate — proving the seed was the prompt, not context bleed (candidate i) or the W122a description (candidate iii, runtime description scanned: no calendar language). **Systemic by construction:** every COLUMN_LOGIC / FUNCTION_LOGIC / VARIABLE_TRACE generation carried the exemplar. Breadth test: 4/4 non-calendar OFSERM functions (FN_G_TEST_CSTM, CS_GROSS_COMMON_EQUITY_TIER1_CAPITAL, T2T_NON_SEC_FSI_CAP_DERIVATIVES, MINORITY_INTEREST_STANDARD_ACCT_HEAD_DATA_POP) received a fabricated gate; one also fabricated a "March 31, 2026" date.

**Fix (prompt-only).** Rule 4 rewritten from the hardcoded December exemplar to a source-predicate-conditional rule: report a calendar/date gate **only when the source contains an explicit month/date predicate** (`EXTRACT(MONTH …) = 12`, `MONTH = 12`, `'DECEMBER'`, or a hardcoded date guard), cite the gating line; **when none is present, assert no calendar/date condition and do not invent a run date** (generalised beyond December to quarter-end / year-end / fixed dates). Rule 7's `(e.g. December-only)` bullet made conditional on a predicate actually being present. No detector touched — **W57 / W83 / W135-137 unchanged**; the fix reduces fabrication at the source while W57 stays the backstop.

**Validation.** Negative isolated probes: 0/4 (was 4/4). Positive: FN_LOAD_OPS_RISK_DATA's REAL gate preserved and now cites the actual `EXTRACT (MONTH …) = 12` line (isolated + live). Live FN_G_TEST_CSTM: `UNVERIFIED`/0.4 → `VERIFIED`/0.95, warnings empty. Quality harness before/after (clean baseline captured on this branch pre-edit, both runs 22/22 ok): fabrication removed on B2/B3/B4/C1/E1/E3; real gates preserved on C3/E4; E1 confirmed fix-working (pre-fix it fabricated "december" on CAP_ACCOUNT_LOAD_RUN_MAP_DATA_POP and W57 flagged it; post-fix it reports that function's real `20260331` literal instead).

**Residual characteristic (expected LLM behavior, not a bug).** The fix dramatically reduced fabrication but did not eliminate it: in the live multi-function retrieval path the LLM occasionally still emits the December gate from its regulatory-domain prior — measured **~6% on C09 (1 of ~17 live runs), 0% on FN_G_TEST_CSTM over 6 runs**. Confirmed **not** context bleed (all 40 functions in the residual C09 run were scanned — none has a real calendar gate). W57 caught the residual run (`UNVERIFIED` / GROUNDING-HIGH). This is defense-in-depth working as designed — the generator does not have to be perfect because the backstop is exact. Driving an LLM domain-prior from ~6% to 0% via prompt wording is unbounded and was deliberately **not** attempted.

**Canaries updated.** `C09` (function has no real gate) replaced its stale `markdown_contains "This function runs in"` assertion — which codified the fabrication — with a compound **W57 safety invariant**: `forbid_markdown_when_field` (new assertion kind in `tests/canary/run_canaries.py`) asserts `NOT (markdown contains "december" AND badge == VERIFIED)`. This passes on both the ~94% clean runs and the ~6% residual runs (W57 flips the badge), and fails only on a *silent* fabrication (December + VERIFIED) — the true regression. Badge pin removed from C09's `expected` (both VERIFIED and W57-caught UNVERIFIED are correct). `C01` (function has a REAL gate) was a wording-drift update: the real December gate is still surfaced, so `markdown_contains "This function runs in"` → `markdown_contains_any ["December","month=12",…]`, and the obsolete `warnings_contain "GROUNDING-HIGH:"` expectation (which had codified the now-removed fabrication catch) was dropped — its `UNVERIFIED` badge now comes via the pre-existing W108-TRUNCATED on the 35-function retrieval, not a fabrication catch.

**Observed flag — canary-suite non-determinism.** During the W148 canary triple, `C12` flipped PASS→FAIL between two runs on identical code; the C09 fabrication itself was 1-of-17. Badge/grounding canary assertions are inherently run-to-run variable, so a single before/after canary diff is not conclusive for attribution — the W148 off-path canary failures (C16/C18 W87 term-routing, C11 DATA_QUERY SQL-gen, C04 writer-enum, C10 W49 partial-source, C08 Phase-2 value-trace) were attributed mechanistically (a prompt edit cannot reach those paths), not by baseline diff.
...
**Follow-up question (generation-side) — RESOLVED.** Why does the explainer fabricate a December gate when the source has no calendar logic? The two candidates posed at discovery were: (a) sibling context bleed from December-gated functions in `multi_source`; (b) prompt-level priming. The isolated single-function probe settled it: the gate appears with FN_G_TEST_CSTM's clean body alone and no siblings, so **(a) is ruled out and (b) is confirmed** — the prompt's hardcoded December exemplar (Rule 4) was the seed. See the "Root cause" and "Fix" sections at the top of this entry.
...
## Alias-qualified INSERT column lists dropped by the parser — FIXED 2026-06-02 (pending reindex; W-number TBD)

**Status:** Parser fix landed in `_extract_insert_columns` ([src/parsing/parser.py](src/parsing/parser.py)); takes effect after a forced loader re-parse (`force_reparse_on_startup`), then `cli.py index --force` for embeddings. Companion to the column-provenance routing fix, which no-ops until this feeds the index. (W-number to be assigned — near-twin took W149/W150 on main.)

**Root cause (two bugs in `_extract_insert_columns`).** OFSAA writes alias-qualified INSERTs — `INSERT INTO STG_PRODUCT_PROCESSOR B (B.FIC_MIS_DATE, B.N_EOP_BAL, ...)`. (1) The column-list regex `INSERT\s+INTO\s+\w+\s*\(` allowed no token between the table name and the column-list paren, so the alias `B` made the match fail entirely → `columns = []`. (2) Even once matched, `B.N_EOP_BAL` was not alias-stripped. `_extract_update_set` already strips `t.COL → COL`; the INSERT path did not.

**Broader implication.** Any column written **only** via an alias-qualified INSERT was invisible to the entire column index (`graph:index:{schema}`) — not just to column-provenance routing, but to anything consuming the index: lineage, variable trace, and grounding's notion of "who writes this column." `N_EOP_BAL → POPULATE_PP_FROMGL` (OFSMDM) was one symptom: the column had **no** `graph:index:OFSMDM` entry at all, so "How is N_EOP_BAL written?" fell back to unanchored semantic retrieval and fabricated a writer. Blast radius is corpus-wide; the fix is verified by an index-diff (edges added, zero edges lost), not the 4-query canary alone.

**Fix.** Widen the regex to `INSERT\s+INTO\s+\w+(?:\s+(?!VALUES\b|SELECT\b)\w+)?\s*\(` (optional alias, with a negative lookahead so `INSERT INTO T VALUES (...)` — no column list — is not mis-parsed as alias `VALUES` + value-list-as-columns), and strip an optional `alias.` prefix from each extracted column. `extract_column_maps`'s `{columns, values, mapping}` contract is unchanged — it is fed correct columns.

**Separate artifacts found, NOT fixed under this ticket (reported):**
- The stray `EOP` entry in `graph:index:OFSMDM` originates from `POPULATE_PP_FROMGL_N8` (a SCALAR_COMPUTE), where a SELECT-expression **alias** (`... N_EOP_BAL EOP`) is captured as a column by `_extract_column_names_from_expr` (builder.py). Different code path from `_extract_insert_columns`; this fix will not resolve it. Candidate for a follow-up ticket.
- MERGE `WHEN NOT MATCHED THEN INSERT (cols) VALUES (...)` does not route through `_extract_insert_columns` at all — `extract_column_maps` dispatches block_type `MERGE` to `_extract_update_set` (the SET path), which finds no SET clause and captures no insert columns. Pre-existing separate gap; out of scope for this fix.

---

## W155. W130 CAP-aggregate bluffs for a CAP unrelated to the named function (F-GT-1) — FIXED (code + unit tests; on `fix/w155-cap-fn-association`, off `origin/main` @ 1132ec8)

**Status:** FIXED — the W130 gate now confirms CAP↔function association; a registry CAP + unrelated named function no longer force-routes to a context-free VERIFIED aggregate. Verified live on a throwaway `:8011` instance of the worktree (the W151 `:8000` / Docker `:8001` were not touched), then committed on `fix/w155-cap-fn-association` and landed on `origin/main` via rebase (linear history, no merge commit). Diagnosis was done up front via the F-GT-1 diagnostic (read-only literal-index probe); not re-derived live.

**Root cause (routing-side, proven).** The W130 pre-classifier hook ([src/main.py](src/main.py), the `detect_named_computation(raw_query, query_type="DATA_QUERY")` call) runs as the FIRST orchestration step in `/v1/stream`, before the LLM classifier. On a registry-CAP regex match it FORCES `state["query_type"]="DATA_QUERY"`, sets `state["w88_pre_detected"]=True`, and BYPASSES the classifier (the `else:` `classify_query` arm is skipped). The named function in the query is never captured — proven byte-for-byte: *"How is CAP169 calculated in FN_G_TEST_CSTM?"* === *"How is CAP169 calculated?"*. `bi_routing` (which WOULD read the named function) never runs because `query_type` is already `DATA_QUERY`. `_build_cap_code_plan` then aggregates the CAP's stored amount with NO function-association check, and DATA_QUERY badging is status+suspicious only (no `evaluate_grounding`), so the answer is unconditionally VERIFIED. Net: *"How is CAP169 calculated in FN_G_TEST_CSTM?"* returned a confident VERIFIED `658963154349.266` even though CAP169 is not computed in FN_G_TEST_CSTM. Bluff scope: the 5 cap_code ANCHOR-arm registry codes {CAP169, CAP090, CAP960, CAP214, CAP192}. CAP013/CAP999 already fall through to an honest UNVERIFIED because they are NOT in the registry.

**Fix (option ii — confirm CAP↔function association at the W130 seam, else fall through).** After `detect_named_computation` returns a match and BEFORE the force-DATA_QUERY arm, gate the cap_code ANCHOR arm only. Two small pure helpers added in [src/agents/orchestrator.py](src/agents/orchestrator.py): `w155_named_functions_in_query` (corpus functions named in the query = `extract_function_candidates` ∩ `function_exists_in_graph`) and `w155_cap_associated_with_named_fn` (whether the canonical registry CAP `defn.filter_code` has any named function among its `resolve_bi_to_function` literal-index candidates). When a real corpus function is named alongside a registry CAP and that CAP is NOT in the named function's candidate set, the hook sets `w88_pre = None` so the classifier + `bi_routing` run (the honest path). `detect_named_computation`, the W88 registry, `bi_routing` internals, `_build_cap_code_plan`'s SQL, the classifier prompt, and DATA_QUERY badging are all UNTOUCHED.

**Why membership, not `resolve is None`.** Empirically every anchor registry CAP RESOLVES to *some* function (CAP169 → ABL_CAPITAL_SOURCE_STANDARD_ACCT_HEAD_DATA_POP, role=filter — NOT FN_G_TEST_CSTM), so `resolve_bi_to_function("CAP169") is None` is FALSE and would not catch the bluff. The correct test is whether the NAMED function is among the CAP's literal-index candidate functions. Verified outcomes: *"CAP169 in FN_G_TEST_CSTM"* → not associated (fall through, fixes F-GT-1); *"CAP960 in CS_COMMON_EQUITY_TIER_1_CAPITAL_RATIO"* → associated (genuine case preserved); *"CAP960 in FN_G_TEST_CSTM"* → not associated (fall through).

**Guardrails preserved.** Bare-CAP aggregate with no function named (*"How is CAP169 calculated?"*) → `named_funcs` empty → gate skipped → aggregate still returned. No-function prose (*"How is the CET1 ratio calculated?"*) → unaffected. Decline arm (LCR/NSFR/Leverage) → unaffected (gate scoped to `arm=="anchor"`). BIA (`method_skey`) → unaffected (gate scoped to `filter_kind=="cap_code"`). `_graph_redis` None → `function_exists_in_graph` returns False → `named_funcs` empty → gate skipped → fail-open (aggregate still returned).

**KNOWN RESIDUAL — deliberately NOT fixed, gate NOT widened.** A query naming a NON-EXISTENT function alongside a registry CAP (e.g. *"How is CAP169 calculated in SOME_FAKE_FN?"*) has `function_exists_in_graph` False → `named_funcs` empty → gate skipped → still aggregates a VERIFIED number. This is an accepted consequence of scoping W155 to real-function + registry-CAP. Widening the gate to catch non-corpus function tokens is explicitly out of scope (it would risk declining legitimate bare-CAP aggregates whose query happens to contain an unrelated function-shaped token). The separate finding D — DATA_QUERY grounding-blind (no `evaluate_grounding` on the DATA_QUERY path), tracked as **W156** — is also out of W155 scope.

**Tests (unit/static).** `tests/unit/agents/test_w130_w88_pre_classifier.py` (detect_named_computation, untouched layer) and `tests/unit/agents/test_w88_computation_router.py` stay green (81 passed). New `tests/unit/agents/test_w155_cap_fn_association.py` (20 passed) covers: registry CAP + unrelated corpus fn → not associated (fall through); registry CAP + its own computing fn → associated (proceed); bare CAP no function → gate skipped (proceed); decline-arm / method_skey match → gate skipped (proceed); `_graph_redis` None → gate skipped (fail-open); membership over full candidate list (not just primary); and the known-residual fake-function case. `python -c "import src.main"` clean.

**Live verification (throwaway `:8011` instance of the worktree binary — confirmed live via the `W155: registry CAP CAP169 not associated` fall-through log signature with matching correlation id).** F-GT-1 (*"How is CAP169 calculated in FN_G_TEST_CSTM?"*) run 5×, fresh session each: **5/5 UNVERIFIED, route VARIABLE_TRACE, `w88_anchor=None`, confidence 0.4** — the gate fired every time and never reverted to the W88 aggregate. P2 (*"How is CAP169 calculated?"*) and CAP960 no-fn → VERIFIED aggregates preserved (CREDIT_RWA_AGG `658963154349.266`; CET1 `0.19286686188`). Genuine *"How is CAP960 calculated in CS_COMMON_EQUITY_TIER_1_CAPITAL_RATIO?"* → still answers (VERIFIED aggregate, association True). CAP999 → UNVERIFIED "Not Found"; CAP013 → UNVERIFIED (cites TLX_PROV_AMT_FOR_CAP013) — both unchanged.

**RESIDUAL (W83x-family follow-up — NOT W155, separate ticket).** After the fall-through, a registry-CAP-in-unrelated-function query lands in the normal VARIABLE_TRACE explainer, which **fabricates** a derivation anchored on the named function (5/5 runs opened with a false *"This entire function ONLY runs when the reporting month is December"* gate and walked a hallucinated 20-step chain through unrelated capital functions). The honest UNVERIFIED badge is **deterministic but carried by the W83x calendar-catch** (`GROUNDING-HIGH`: unsupported December template) firing every run — **not** by a clean *"CAPxxx is not computed in FN_X"* decline. (The advisory `GROUNDING-LOW` line-padding warning flickered 3/3/?/?/3 — count varies, badge does not.) If a future fabrication tripped no HIGH detector, the badge could revert to VERIFIED; honesty here rests on the calendar net, not on W155. Same surface as **G-Test F-GT-3** (March/December gate invention) — cross-linked. A clean-decline fix (make the fall-through emit *"CAPxxx not computed in FN_X"* rather than a fabricated walkthrough) belongs to the calendar / VARIABLE_TRACE-hallucination ticket, not W155. The W46 ValidationHeader (`TrustBanner`) does render the Unverified badge + the GROUNDING-HIGH flag ABOVE the fabricated body, so the user-facing surface is "Unverified, with the unsupported-December flag, over the walkthrough."

---

## W168. Badge anti-correlated with correctness — anchor-vs-prose attribution gate — SHIPPED 2026-06-08

**Status:** SHIPPED on `fix/w168-attribution-mismatch-gate` (off `91bec4e`, W164). Implementation commit `7a19ea7`; landed on `origin/main` via throwaway-worktree fast-forward (linear, no merge commit; primary checkout + the `:8000` backend untouched). Diagnosed read-only first (Phase-1a design review), then route-validated and live-validated before commit.

**Failure surface.** The adversarial run found the VERIFIED badge anti-correlated with correctness: wrong/fabricated answers shipped at VERIFIED·0.95 when the function the pipeline ROUTED to was absent from the prose the answer actually wrote. The routing anchor (`state["w70_anchor"]`, w76 fallback) and the prose's cited-function set were both in scope at badge time in `evaluate_grounding` but never compared.

**Fix (anchor-vs-prose gate, badge-cap only).** New standalone `_w168_check_attribution_mismatch` ([src/agents/logic_explainer.py](src/agents/logic_explainer.py)), called from `evaluate_grounding` after the W57 block and before `blocking_warnings`, so its `GROUNDING-ATTRIBUTION-MISMATCH-HIGH` warning flips the badge to UNVERIFIED·0.4 through the same machinery as W85 — no badge-formula or payload change. Fires iff ALL: (1) `query_type != VARIABLE_TRACE`; (2) an anchor resolves; (3) anchor `source` is EXPLICIT (`w76_*` / `classifier_object` / `bi_routing` / `raw_query_scan` — `semantic_top1` and unknown sources suppressed, the same weak-anchor signal W150 keys on); (4) the prose cites ≥1 function; (5) `normalize(anchor)` (via `SchemaAwareKeyspace.normalize_function_name`) is neither in the prose cited-function SET nor a substring of the normalized body (the substring belt). All three design open-questions decided conservatively: explicit-source allow-list (fail-safe suppress on future sources), VARIABLE_TRACE excluded (W159 fan-in must-not-regress), set-membership + substring belt (a false-positive on a trust gate is worse than a Phase-2-backstopped miss).

**CORRECTED CATCH-SET (live falsified the design's broader claim).** W168 catches ONLY **anchor-ABSENT / "silent-swap"** drift — the routed function name appears NOWHERE in the prose. Proven live: *"How is CAP973 calculated?"* → UNVERIFIED·0.4 (`bi_routing` anchor CS_REGULATORY_ADJUSTMENTS_PHASE_IN_DEDUCTION_AMOUNT absent; prose describes REGULATORY_ADJUSTMENT_STANDARD_ACCT_HEAD_DATA_POP). It does **NOT** catch the adversarial **Q12/Q16/Q48** (*"how is N_BASEL_ASSET_CLASS_SKEY updated in FN_G_TEST_CSTM"*) — see **W169**. The Phase-1a design report and an earlier draft of the design-doc §6 matrix wrongly listed Q12/Q16/Q48 as caught; corrected in [docs/w168_attribution_mismatch_gate.md](docs/w168_attribution_mismatch_gate.md) §6/§7.

**Live validation (`:8000` on the branch).** True positive: CAP973 fires. Zero false positives: Q1/Q2 (N_STD_ACCT_HEAD_AMT fan-in) stay VERIFIED·0.95, Q9 (broad goodwill) UNVERIFIED from other checks with no W168 warning. Canary tier-1 C01/C19 VERIFIED. The C16/C18 tier-1 failures are **pre-existing and unrelated** — stash-verified by re-running them on a clean-base backend (`:8002`, throwaway; `:8000` untouched): normalized-identical failures (unrecognized_term path, `query_type=None`, W168 never invoked).

**Tests.** `tests/unit/agents/test_w168_attribution_mismatch.py` — 26 pass, incl. the grounding-machinery test (warning → `blocking_warnings` → UNVERIFIED → 0.4, mirroring W85). Full unit suite 1687 pass / 5 pre-existing fail.
...
**Surface.** Q12/Q16/Q48 from the adversarial run, e.g. *"how is N_BASEL_ASSET_CLASS_SKEY updated in FN_G_TEST_CSTM"*. Live: the prose leads *"In the function FN_G_TEST_CSTM, the variable … is updated … primarily in the ABL_INV_ASSET_CLASS_RECLASS and FN_COR_RW_U2_CSTM functions"* — i.e. it names the routed anchor as **framing context** while the substantive description is a **different** function. Ships VERIFIED·0.95.

**Why W168 is structurally blind.** W168 fires on `anchor ∉ prose`. Here the explainer frames the answer with the anchor's name, so `anchor ∈ prose` and W168's set-membership predicate (kept lenient on purpose so Q39 two-function comparisons stay VERIFIED) stays silent — independent of the VARIABLE_TRACE gate (this query also routes VARIABLE_TRACE, a second independent reason).

**Two sub-problems.**
1. **Extractor gap (NAME-outside-parens).** The real subjects are cited as `**ABL_INV_ASSET_CLASS_RECLASS (Lines 32-125)**` — function name OUTSIDE the parens — which none of the three W57 extractors (`_W57_PROSE_FUNCTION_REF_RE`, `_W57_HEADING_AND_RESPONSIBILITY_REF_RE`, `_W57_FUNC_CITATION_RE`) match, so they never enter the prose set at all.
2. **Dominant-subject test reopens FP risk.** Catching framing-drift needs moving from set-membership to a *dominant-subject* test (anchor present but not the dominant cited subject → fire), which reopens the exact Q39 (legit two-function compare) and W159 (fan-in) false-positive surface W168's set-membership predicate was chosen to avoid.

**Diagnosis (two read-only probes, live-confirmed).** Check 3a `_w57_check_anchoring` runs to completion on Q12 — no early bail — and fails the 2× frequency comparison at `:2371` because the predicate is degenerate in the sparse-citation regime: Q12 cites the named scope and the drifted function **once each** (a 1–1 tie), so `1 > 2` is false. The miss is also **non-deterministic** — a verbose run that repeats the wrong function ≥3× *is* caught by the same line; only the short-prose regime ships VERIFIED·0.95. The recoverable signal is the per-function **attested writer spans** for the traced column, computed before grounding and read from the same structured fan-in source the diagram is built on (`vt_graph` via `fan_in_steps_from_graph` first, `vt_tagged` fallback). Raw line-range ownership is degenerate (file-local numbering) and prose verb-attribution is non-deterministic; the structured writer set is the durable signal. The discriminator — *prose attributes the write to a function OUTSIDE the query's named scope, while the named scope is itself an attested writer* — was confirmed FP-safe against three legitimate two-function comparisons (both functions in-scope → silent).

**Fix (Check-3a strengthening, route-split).** New pure predicate `_w169_attribution_drift` (column-/line-range-first, prose-name fallback; never raw frequency) emits `GROUNDING-ANCHOR-SCOPE-DRIFT-HIGH`, flipping the badge to UNVERIFIED via the existing blocking-warning machinery. Wired into `_w57_check_anchoring`: when the attested-writer set is present (VARIABLE_TRACE — threaded from `main.py` through `evaluate_grounding` → `w57_enforce_grounding`) the scope-drift predicate **replaces** the frequency rule; when absent (FUNCTION_LOGIC / COLUMN_LOGIC, no `vt_*` signal) the **legacy frequency rule is preserved** so the Run-3 C2 dominance benchmark does not regress. The attested-writer set is built by a thin `attested_writers_for_target` accessor in `trace_diagram.py` that composes the existing projections (single-sources the writer-op definition). Disjoint from W168 (anchor-absent) and W85; does not touch the W108 truncation path.

**Implementation divergences from the original frame (surfaced before coding, both confirmed live).** (1) The frame scoped the signal to `vt_tagged`; live, Q12's diagram is built from `vt_graph` (graph path runs first), so the accessor reads **both**, graph-first, mirroring the diagram's own precedence. (2) The frame said "replace, don't stack" the frequency rule, but an existing benchmark-backed test (`test_check3a_fail_dominant_other_function`, Run-3 C2) depends on it firing on a FUNCTION_LOGIC case W169's structural predicate cannot cover (no `vt_*` signal there) — so the replacement was **route-scoped** (VARIABLE_TRACE only) rather than global, preserving that catch.

---

## W167. Synthesizer drops secondary writers — wide writer fan-in silently truncated, prose claims completeness — RECLASSIFIED MEDIUM → HIGH; FIXED (payload projection + C12 re-pin; held for review)

**Status:** FIXED on `fix/w167-payload-fanin-honesty` (off main `906186d`). Two `src/` changes (`assemble_llm_payload` honest truncation in `query_engine.py`; one threaded arg in `main.py`), 6 unit tests, C12 canary re-pinned, this log. No diagram path, W169/W172/W153, badge logic, prompt, or frontend touched. HELD for review of the verbatim post-fix N_STD_ACCT_HEAD_AMT prose (the completeness framing is the thing to eyeball) and the C12 re-pin. A read-only containment-first diagnostic preceded it; this builds on it.

**Reclassified MEDIUM → HIGH (trust, not quality).** Logged MEDIUM ("quality, not trust") with the framing "quaternary reported as binary." The diagnostic overturned both: a column with **90 attested writers** has its prose report only **4** and frame them as the COMPLETE writer set ("`N_STD_ACCT_HEAD_AMT` is written by multiple functions, specifically … the following functions: 1–4 … In summary … across these functions"), badge **VERIFIED**, no warning. The badge affirms a completeness claim that is **4/90ths true** — a trust defect. The logged magnitude understated it by an order of magnitude (90→4, not 4→2).

**Surface (live @ `906186d`, deterministic 3/3).** `What writes N_STD_ACCT_HEAD_AMT?` → VARIABLE_TRACE → GRAPH branch (`stage=graph_path_selected`, `branch=graph_payload`). Diagram holds **90** write-attested writers (all verified genuine inserts of the column); prose names **4**; badge VERIFIED; no warnings.

**Root cause — NOT the eval's "A1" theory.** The eval hypothesised the prose ran over corpus-wide unscoped `multi_source` ([main.py:1894](src/main.py#L1894), the raw-source/VARIABLE_TRACE branch). That is **not** the live mechanism. The repro is on the GRAPH branch, where prose comes from `assemble_llm_payload(fetched_nodes)`, not `multi_source`. The mechanism is a fixed length cap: `_MAX_PAYLOAD_CHARS = 4000` ([query_engine.py:758](src/parsing/query_engine.py#L758)) truncated the payload with a blind string slice (`payload[:CAP-20] + "... [truncated]"`), so only the first ~4 writer STEPs fit; the rest were silently amputated. The diagram, built from the same `fetched_nodes` via `fan_in_steps_from_graph` (W153 attestation, **no** length bound, [main.py:2110-2131](src/main.py#L2110-L2131)), kept all 90. Offline replay reproduced `payload_chars=3996` / 4 surviving steps exactly, and the 4 surviving = exactly the 4 the prose named → **the LLM is faithful to its truncated input; the loss is upstream, in the payload projection.** The eval's A1 raw-source path is largely unreachable for indexed columns (graph resolution returns nodes → graph branch wins).

**Fix (payload-projection layer, count-aware honest truncation).** `assemble_llm_payload` gains an `attested_writers` arg — the SAME write-attested set the diagram is built from (`attested_writers_for_target` → `fan_in_steps_from_graph`, threaded from [main.py:1693](src/main.py#L1693), computed from `fetched_nodes` + the already-populated `multi_source` cohort). When the payload overflows budget AND the surviving steps cover fewer than the true writer count N, a **top-anchored** `WRITER COMPLETENESS NOTICE` (survives the tail truncation) states the true count, directs the model to state it, and labels the shown steps a subset: *"`N_STD_ACCT_HEAD_AMT` is written by 90 attested functions in total … Your answer MUST state that `N_STD_ACCT_HEAD_AMT` is written by 90 functions. Only 4 of 90 writers are detailed below … this is a SUBSET, NOT the complete list. Do NOT state or imply that the writers shown are the only ones."* N == diagram writer count **by construction** (same source). The 4 KB budget stays — truncation becomes honest, not removed. The cut is taken at a section boundary so whole writer records survive and M (shown) is countable; ordering is the existing deterministic `determine_execution_order` topological sort (writers were already non-arbitrary — no ordering change needed). When no attested-writer signal is supplied (single-function logic walks), behaviour is unchanged (plain `[truncated]`).

**Live validation (@ `906186d`, post-restart).** Before, deterministic 3/3: *"…specifically … the following functions: 1–4 … In summary … across these functions"* (4 framed as complete). First fix cut: *"written by multiple functions, with the principal writers being: 1–4"* — completeness claim gone, but the model said "multiple" and omitted the count; the notice was therefore strengthened to direct stating the total. After, deterministic 3/3: *"**N_STD_ACCT_HEAD_AMT is written by 90 functions in total. Among these, the following four functions are principal writers:**"* — true count stated, shown set labeled a subset, badge VERIFIED, diagram still 90. Regressions: narrow 2-writer column (`N_RWA_AMT`) lists both writers, no notice; `How does FN_G_TEST_CSTM work?` (FUNCTION_LOGIC / W172 branch) unchanged, no notice (not a column writer-trace); C12 re-pin passes live 3/3; C19 (W172/W160) unchanged; full unit suite shows only 5 pre-existing unrelated failures (confirmed on main).

**Orthogonality (confirmed, untouched).** W169 stays silent on the under-report (it fires only on attribution OUTSIDE a *named* scope; `asked` is empty for a bare column query → returns None at [logic_explainer.py:2370](src/agents/logic_explainer.py#L2370); dropping in-scope writers is not its case). W172 independent (column query reaches graph on its own; rescue fires only when `target_var` AND `obj_name` are both empty). W153 attestation reused, not weakened — N is the attested-writer count, never the raw node count. The W169 grounding computation at [main.py:1985](src/main.py#L1985) is left byte-identical; W167 computes its writer set separately (keyed on `g_search_term`, the payload's own target) rather than reuse it, to avoid perturbing the orthogonal gate — the helper is pure and cheap.

**Tests.** 6 unit tests in [tests/unit/parsing/test_query_engine.py](tests/unit/parsing/test_query_engine.py) (fixtures at the real N=90 magnitude): wide fan-in states true count + subset framing and stays ≤4000; narrow (N=3) lists all, no subset framing; single writer unchanged; truncation deterministic; non-writer node not counted in N; no-attested-writers preserves legacy `[truncated]`. All 6 + 12 pre-existing assemble tests green; 356 parsing+trace_diagram tests green.

**C12 reconciled here (same query, cannot validate W167 against a stale canary).** C12 (`What writes N_STD_ACCT_HEAD_AMT?`) pinned `badge: UNVERIFIED` + `GROUNDING-HIGH`, describing a wrong-family retrieval that no longer occurs (live: VERIFIED 3/3, the SEC writers retrieved). Re-pinned to the post-W167 structural invariants (route VARIABLE_TRACE/COLUMN_LOGIC, writer present, honest-count framing), badge NOT pinned (W171 lesson). See the C12 before/after in the implementation report.

---

## W167-related watch-item. C04 (`What writes N_EOP_BAL?`) has drifted — W150 near-twin hedge now intercepts it (record only, NOT W167 work) — RESOLVED under W173 (see the W173 entry below)

Surfaced during the W167 diagnostic (live @ `906186d`, 2/2 runs). C04's canary expects VERIFIED + `FN_LOAD_OPS_RISK_DATA`, but `What writes N_EOP_BAL?` now short-circuits to the **W150 near-twin disambiguation hedge** (anchor `POPULATE_PP_FROMGL`, margin 0.0000, 5 siblings) → clarification, no diagram, no writer enumeration → UNVERIFIED. The classifier still emits VARIABLE_TRACE / OFSMDM / conf 0.95, but the W150 hedge fires before the writer path. Diverges from the C04 expectation; out of scope for W167 (a routing/hedge-threshold question, not a payload-projection one). Flag for a separate look — do NOT fold into W167.
...
## W171. Badge-pinned canary flaky against generator non-determinism — FIXED (canary re-pin; held for review)

**Status:** FIXED on `fix/c19-flaky-canary-invariants` (off main `ab12293`). Canary-definition + log only — no `src/`, no prompt, no W57 check touched. HELD for review of the before/after C19 definition before commit. This was the actual yield of the W170 hunt: W170 found no framing-drift repro, but the sweep surfaced that the C19 canary itself was mis-pinned. C19 now pins ONLY the four W160 structural invariants (`query_type==COLUMN_LOGIC`, no `W108-TRUNCATED`, no `UNGROUNDED_IDENTIFIERS`, `type!=function_not_found`); BOTH the badge and `min_citations` were dropped as floating metrics.

**Surface.** Canary C19 (`tests/canary/canaries.yaml`, "how does FN_G_TEST_CSTM work") pinned `badge: VERIFIED` under `expected:`. The badge is a **non-deterministic function of generator phrasing**, not a stable invariant: across 17 live `/v1/stream` runs @ 43de8b7, ~24% (4/17) the LLM emitted a "pass-through" / "copies data unchanged" characterization of FN_G_TEST_CSTM, observed verbatim as *"### Step 1: Pass-Through of Values (Lines 69-1009) … primarily serves as a pass-through mechanism … without transforming the values. It copies data unchanged …"*. FN_G_TEST_CSTM is an 8-INSERT / 3-MERGE retail-granularity reclassification ETL (MERGE `WHEN MATCHED THEN UPDATE SET` clauses carry CASE arms with arithmetic like `N_RET_EXP_TOTAL / LN_RET_TOTAL > LN_RTL_GRAN`) — the opposite of a pass-through. W57 Check-5 (`_w57_check_template_phrases` → `_w57_supports_passthrough`) correctly rejects the claim → UNVERIFIED·0.2. Steady state on the other 12/17 is VERIFIED·0.95, warnings empty.

**Root cause — the canary pinned the wrong thing.** The W57 check is **correct** (predicate run offline against the real source returns `False`; 8 `INSERT INTO`, 3 transform-MERGEs). This is the same defense-in-depth backstop firing as the documented W148 December residual (an LLM domain-prior flourish the prompt fix reduced but, by deliberate decision, did not drive to 0% — see W148, "unbounded and deliberately not attempted"). C19's real purpose (added W160 `9b93e11`, refined W163 `b712d65`) is to protect the **W160 single-function retrieval collapse** — `query_type==COLUMN_LOGIC`, no `W108-TRUNCATED`, no `UNGROUNDED_IDENTIFIERS`, citations present — none of which depend on the badge. Likely flaky since introduced at W160; never caught because prior validation tracked the *December* phrase, not *pass-through*.

**Fix (canary-level only).** Dropped TWO floating assertions from C19: `badge: VERIFIED` (from `expected:`) and `min_citations: 1` (from `assertions:`). Kept the four W160 structural invariants — `query_type: COLUMN_LOGIC` (`expected:`), `warnings_not_contain W108-TRUNCATED`, `warnings_not_contain UNGROUNDED_IDENTIFIERS`, `done_field_neq type function_not_found` (`assertions:`). The badge was **not** replaced with a "tolerate VERIFIED-or-UNVERIFIED" form. The W57 check, the `>500`-span citation guard, every other canary, and all prompts were left untouched.

**Why `min_citations` had to go too (the STOP finding that expanded the fix).** Validation across 52 live runs @ `ab12293`: the residual reproduced on **3** (≈6% here; was ≈24% / 4-of-17 @ 43de8b7 — the rate is itself non-deterministic). On the 49 VERIFIED runs all metrics held (`source_citations` 739–899). On the 3 residual UNVERIFIED-pass-through runs the four structural invariants held — **but `source_citations` was 0, so the original `min_citations: 1` FAILED on all 3.** `min_citations` only *looked* structural: `evaluate_grounding` sets `source_citations = _extract_line_citations(markdown)` (line-refs parsed from the *prose*), and `_extract_line_citations` discards any range with `end - start > 500` ([logic_explainer.py:604](src/agents/logic_explainer.py#L604)). The degenerate residual prose cites the whole body as one mega-range *"### Step 1: Pass-Through of Values (Lines 69-1009)"* — span 940 > 500 — so its only line-ref is dropped, leaving zero. (The hard CITATIONS block does not fire because `functions_analyzed` stays populated; the UNVERIFIED comes from the blocking pass-through GROUNDING-HIGH, confidence 0.2 because the extracted-citation list is empty.) So the SAME degenerate generation that trips the (correct) pass-through check also produces zero valid citations: badge and `min_citations` float **together** with prose shape. A `functions_analyzed`-based grounding guard was rejected — it stays populated on the residual, so it is a *retrieval* guard, not a *grounding* guard, and would have needed a new harness assertion kind outside canary scope.

**Watch-item (record only, NOT active work).** The residual's mega-range *"(Lines 69-1009)"* citation being silently dropped by the `>500`-span guard is the same W148-class residual flourish the log already declared not-worth-chasing (driving the LLM domain-prior to 0% via prompt wording is "unbounded and deliberately not attempted"). Logged here for findability only; do not open as active work unless the residual rate climbs materially or a real query starts losing legitimate citations to the span guard.
...
## W172. "How does <FN> work" binds to whole-query variable → raw-source instead of graph — FIXED (binding rescue; held for review)

**Status:** FIXED on `fix/w172-binding-function-rescue` (off main `828ded9`). One change in `src/main.py` `resolve_graph_binding` `(else)` case + tests. No classifier/orchestrator change, no prompt, no W57/W108/W169/W45/W49/W150 path touched. HELD for review of the branch-flip + schema-repoint evidence before commit. Four read-only diagnostic passes preceded it (fork localization → binding-split → classifier capture); this builds on them, not re-litigated.

**Surface.** A `FUNCTION_LOGIC` / `COLUMN_LOGIC` query that NAMES a real, graph-resident function ("How does FN_LOAD_OPS_RISK_DATA work?", "how does FN_G_TEST_CSTM work") fell to RAW-SOURCE generation — the W108-capped unscoped multi-function concat — instead of the GRAPH branch, even though the function is parsed and present. Live proof (9/9 runs @ `828ded9`): `CS_Deferred_Tax_Asset_Net_of_DTL_Calculation` → `target_variable` set → W164 function branch → GRAPH; `FN_LOAD_OPS_RISK_DATA` & `FN_G_TEST_CSTM` → `target_variable=None` → `(else)` → 0 nodes → RAW-SOURCE. Same query shape, opposite outcome.

**Root cause (fully diagnosed, generation-orthogonal).** The `stream_semantic` fork ([logic_explainer.py:3963-3964](src/agents/logic_explainer.py#L3963-L3964)) is ALREADY graph-first; it falls to raw-source only when `state["llm_payload"]` is empty, which on a healthy backend means `resolve_query_to_nodes` returned 0 nodes ([main.py:1685,1717](src/main.py#L1685)). That happens because `resolve_graph_binding` ([main.py:2540](src/main.py#L2540)) degrades to the `(else)` `return "variable", raw_query, g_schema` ([main.py:2637](src/main.py#L2637)) — the WHOLE query string looked up in the variable index, a guaranteed miss. It reaches `(else)` because BOTH `target_var` and `obj_name` are empty: the LLM classifier deterministically WITHHOLDS `FN_`-shaped names from `target_variable` (its `intent` reads them as "the PL/SQL function FN_…", not a variable-to-trace), and `object_name` is set only by W76 prefix / bi_routing ([orchestrator.py:540,1243](src/agents/orchestrator.py#L540)), neither of which fires for "how does <FN> work". Documented run-to-run flicker on top ([main.py:2690-2696](src/main.py#L2690-L2696)). So the classifier is not a dependable carrier of the function identity for this shape — the fix must not depend on it.

**Fix (deterministic function-existence rescue in the `(else)`).** Before the unchanged `return "variable", raw_query, g_schema`, extract candidates from `raw_query` (`extract_function_candidates`) and, on an EXACT graph-key hit (`function_exists_in_graph` — case-insensitive, fails-open `False`), bind as a function: normalize to the canonical key form (`SchemaAwareKeyspace.normalize_function_name`) and repoint `g_schema` via `schema_for_function`, mirroring W164's branch. Logs `[W172] function-existence rescue: …`. On no candidate or a non-exact candidate, falls through to the unchanged whole-query-variable default. **EXACT-only by design** — `find_similar_function_names` (the fuzzy W166 helper) is deliberately NOT used; a near-miss must fall through, never force-bind a wrong function (W166 wrong-anchor containment).

**Folded-in schema fix (same root, same change).** `FN_G_TEST_CSTM` mis-classifies `schema=OFSMDM` but is OFSERM; the `(else)` lookup was a DOUBLE miss (wrong field shape AND wrong schema). The rescue's `schema_for_function` repoints the BINDING schema (`g_schema`) to OFSERM so the graph lookup hits `graph:OFSERM:FN_G_TEST_CSTM` (live: 40 nodes resolved where 0 resolved before) and `cited_schemas` carries OFSERM — exactly as W164 does for the target_var path ([main.py:2612-2614](src/main.py#L2612-L2614)). NB: the top-level `data.schema` payload field stays the classifier's primary anchor (OFSMDM here) — the pre-existing W91/W92 anchor-vs-cited split, unchanged and out of W172 scope.

**W164 disjointness (containment, confirmed).** W164's function-identity branch is nested under `if target_var:` ([main.py:2588-2624](src/main.py#L2588-L2624)) and fires only when the classifier ECHOED a known function into `target_variable`. W172 is the disjoint `target_var`-AND-`obj_name`-empty `(else)` path; the rescue never runs when `target_var` is set. Same machinery reused (extract + exact-existence + normalize + schema_for_function), third entry condition. `resolve_graph_binding` is called only at [main.py:1645](src/main.py#L1645) (the `/v1/stream` path); `explain_semantic`'s identical fork on the non-streaming `/v1/query` path has no `llm_payload` producer at all (pre-existing, not source-of-truth per CLAUDE.md) — deliberately out of scope, not expanded.

**Tests.** 13 unit tests in [tests/unit/test_w172_binding_function_rescue.py](tests/unit/test_w172_binding_function_rescue.py) (mirrors the W164 monkeypatch harness): rescue binds exact graph function; schema repoints OFSMDM→OFSERM; `candidates[0]` convention; mixed-case normalization; xyzzy no-candidate falls through (probe never runs); near-miss non-exact candidate falls through (probe runs, rejected); fuzzy matcher never called (fixture raises if it is); genuine variable stays variable; W164 path intact when target_var set (rescue not reached); case-(d) unknown target_var unchanged. All 13 + 12 pre-existing W164 tests green. 5 integration cases in [tests/integration/test_live_stream.py](tests/integration/test_live_stream.py) keyed on a new `branch_for_query` log reader (the fork is only observable in the server log): FN_LOAD → graph; FN_G_TEST → graph + schema OFSERM (pins branch+schema, NOT the W171-flickering badge); CS_Deferred_Tax still graph via W164; unknown function still DECLINE; variable trace unchanged.
...
## W173. W150 near-twin hedge misfires on colprov-injected score-0.0 sentinels (the C04 interception) — FIXED (hedge seam + C04 re-pin; held for review)

**Status:** FIXED on `fix/w173-colprov-hedge-seam` (off main `486eda9`). One helper + one exclusion in [src/agents/anchor_resolution.py](src/agents/anchor_resolution.py), C04 re-pin in `tests/canary/canaries.yaml`, unit tests in `tests/unit/agents/test_w149_w150_near_twin.py`. No threshold, predicate, injection-path, prompt, or badge-logic change. HELD for review. Two read-only diagnostic passes preceded it (the C04/C05 drift localization and the W173 pre-fix discriminator probe); this builds on them.

**Surface.** `What writes N_EOP_BAL?` (canary C04) short-circuited to the W150 near-twin disambiguation hedge — `anchor=POPULATE_PP_FROMGL margin=0.0000 siblings=5`, UNVERIFIED·0.2, no writer enumeration — instead of the correct, complete answer. Live-proven 3/3 @ `486eda9`, byte-identical (deterministic).

**Root cause (proven, two-commit integration gap).** `apply_column_provenance_anchor` resolves a named column's writers deterministically from the column index; `ensure_column_writers_in_search_results` force-injects missing writers with a hardcoded `score: 0.0`. A column query has no L1–L4 anchor layer, so L5 `semantic_top1` takes `min(score)` — an injected 0.0 beats every genuine cosine hit — and `detect_near_twin_ambiguity` then compared the two injected sentinels: `margin = 0.0 − 0.0 = 0.0000`, near-twin name predicate true (`POPULATE_PP_FROMGL` / `_AMC`), hedge fired. The 0.0000 "tie" was never an embedding similarity. The falsified assumption was the L5 comment ("injected score-0.0 entries resolve at L1/L4 before this layer") — true for W95/W147, false for colprov. W149+W150 (`fdfb3f4`) merged ~25 minutes before colprov (`ff0cf65`) landed on top of it; the colprov injection invalidated W150's input assumption the same day it was written. **Latent class:** an index scan found **101 columns** whose attested writer set begins with a near-twin pair — every one would have hedged identically; N_EOP_BAL was just the only one under canary watch.

**Fix (input-set exclusion only; discriminator pinned by the pre-fix probe — verdict (c), both conjuncts load-bearing).** New pure helper `_is_colprov_injected_sentinel(record, function_name, column_provenance)`: True iff `score == 0.0` AND the name is case-insensitively in `column_provenance["writer_functions"]`. `detect_near_twin_ambiguity` skips such records when building the candidate list, so the margin is computed over genuine cosine candidates only; when exclusion leaves <2 genuine candidates (the C04 case) the existing `len(real) < 2` branch yields no hedge and the query proceeds to writer enumeration. Why both conjuncts: `score == 0.0` alone is fragile (`fetch_multi_logic` defaults a *missing* score to 0.0 — currently unreachable since every search_results producer sets score explicitly, but a latent trap); membership alone is wrong (a writer can arrive as a genuine cosine hit with a real score and must keep participating in the margin). The per-record `anchor_injected: True` flag was unusable — `fetch_multi_logic` strips it; only six fields survive into `multi_source`. Non-colprov queries have no `column_provenance` → helper inert → byte-identical behavior.

**C04 re-pin (bundled — the old pin was independently stale).** `FN_LOAD_OPS_RISK_DATA` does NOT write N_EOP_BAL (its 377-line source touches it only in a commented-out declaration `--- ld_n_eop_bal NUMBER (22, 3);`; its attested write target is `N_ANNUAL_GROSS_INCOME`, UPDATE). The content pin was already failing at W148 — before W150 existed (`scratch/w148_canary_tier2.txt`: UNVERIFIED, FN_LOAD_OPS_RISK_DATA absent) — so no commit "broke a green C04"; the pin captured a pre-colprov LLM conflation. Re-pinned (C19/C12 pattern): kept `query_type: VARIABLE_TRACE` + `schema: OFSMDM` (colprov stamps schema deterministically when all writers share one), dropped `badge: VERIFIED` (W171 lesson — floats with generator phrasing), assertions now `markdown_contains POPULATE_PP_FROMGL`, `warnings_not_contain W150-NEAR-TWIN-AMBIGUOUS`, `warnings_not_contain UNGROUNDED_IDENTIFIERS`, `min_citations 1`.

**Validation.** Unit: 35/35 in `test_w149_w150_near_twin.py` (20 pre-existing W149/W150 + 15 new W173 incl. the load-bearing-conjunct test and genuine-tie-still-hedges); `tests/unit/agents/` 1220 passed (1 pre-existing W148-era prompt-text failure, fails on pristine main too). Live @ fixed backend: C04 3/3 no hedge, VERIFIED·0.95, warnings [], markdown names BOTH writers with line cites ("`POPULATE_PP_FROMGL` … Line 157; `POPULATE_PP_FROMGL_AMC` … Line 10"); **genuine near-twin regression guard** — the W150-acceptance CAUGHT-MISS query ("minority-interest capital … included in additional tier one capital") still hedges, `margin=0.0076` (a real cosine margin), W150-NEAR-TWIN-AMBIGUOUS emitted; C12 VERIFIED no-hedge; C05 VERIFIED exact sum. Canary runner: C04 / C05 / C12 all PASS. **STEP-3 finding: no L5 change was needed** — with the hedge suppressed, C04 renders the writer answer through the same path C12 already used.

**C12 iteration-order fragility — CLOSED by this fix (watch-item resolved).** Pre-W173, C12 (`N_STD_ACCT_HEAD_AMT`, 90 writers, all injected at 0.0) survived only because its first two writers in iteration order are not name-twins; positions 2–5 contain two perfect twin pairs (`SEC_PRE_CRM_RWA_EXP_…_IRB/_STD`, `SEC_RWA_EXP_…_IRB/_STD`), so a manifest/ordering change could have flipped it into hedging. Post-W173 ALL colprov-injected writers are excluded from the margin regardless of order, so no arrangement of injected sentinels can produce a hedge — the 101-column latent class is closed **at the hedge**. Confirmed live: C12 VERIFIED, no hedge.

**DEFERRED (own future ticket — needs its own diagnostic-confirmed scope): L5 sentinel-anchoring on the colprov path.** `determine_primary_anchor` L5 still selects `min(score)` over `multi_source`, so on EVERY colprov query the semantic_top1 anchor lands on an injected 0.0 sentinel — observed live on both cross-check columns even when no hedge fires (`apply_w70_anchor: anchored on T2T_FCT_CCP_DETAILS_STD_ACCT_HEAD_POP (source=semantic_top1)` for N_STD_ACCT_HEAD_AMT; `FN_LOAD_OPS_RISK_DATA` for N_ANNUAL_GROSS_INCOME). Harm is low (the anchor is usually a real writer; the low-confidence block lets the explainer roam; answers render correctly) but `w70_anchor` diagnostics on colprov queries are systematically mislabeled as embedding-derived. The W173 caveat comment at the L5 `_score` site marks it. Do not fold into W173. **Dispositioned under W174 (see below): BENIGN, do-not-fix.**

---

## W174. L5 semantic_top1 anchor on colprov queries is an injection sentinel, not an embedding rank — BENIGN, KNOWN INTERNAL-DIAGNOSTIC INACCURACY — DO NOT FIX (W165 pattern)

**Status:** CLOSED as benign-by-evidence after the dedicated read-only diagnostic @ live main `e144f9f` (2026-06-11; 4 columns, 8 live `/v1/stream` runs incl. stability re-runs, plus a deterministic offline rebuild of the graph LLM payloads). This is the W173-deferred follow-on, now dispositioned. No fix written; no fix planned. Re-open ONLY on the revisit conditions at the bottom.

**WHAT (confirmed live, 4/4 queries).** On every column-provenance query where at least one resolved writer was missing from vector retrieval (the common case — all 4 probes), `ensure_column_writers_in_search_results` injects the missing writers at hardcoded `score: 0.0`, and `determine_primary_anchor` L5 ([anchor_resolution.py:179-218](src/agents/anchor_resolution.py#L179-L218); `min()` at :207) picks an injected sentinel — a 0.0 beats every genuine cosine distance. Proven sharply by `N_ALLOCATION_RANK`, where `ALLOCATION_RANK_ASSIGNMENT` arrived as a GENUINE cosine hit yet a sentinel still won. The result is stamped `source=semantic_top1, confidence=low` in the `apply_w70_anchor` log line and surfaces on the wire as `done.diagnostic.w70_anchor` (function name only; source/confidence are log-only). **The label is wrong** — the pick is a deterministic injection artifact, not an embedding ranking — but the anchored function was a genuine attested writer of the asked column in every observed run. Anchors observed: `POPULATE_PP_FROMGL` (N_EOP_BAL), `T2T_FCT_CCP_DETAILS_STD_ACCT_HEAD_POP` (N_STD_ACCT_HEAD_AMT), `FN_LOAD_OPS_RISK_DATA` (N_ANNUAL_GROSS_INCOME), `T2T_SUB_EXPOSURES_FCT_SUB_EXPOSURES` (N_ALLOCATION_RANK).

**WHY BENIGN (the harm assessment that justified W173's "lower harm" call).** Colprov queries stream through the graph-payload branch ([main.py:1893-1899](src/main.py#L1893-L1899) → `stream_semantic` with `state["llm_payload"]`): the prose prompt is `assemble_llm_payload`'s output — graph nodes, exec order, W167 attested writers — **with zero anchor input**; the anchor reaches the LLM only as the soft W70 `_LOW_BLOCK`, which the prose demonstrably overrides (the 90-writer answer never even mentions its anchored function). Badge machinery is untouched (W168 no-ops on VARIABLE_TRACE + semantic_top1; W85 no-ops because column queries name no function; W169 attested writers come from the graph, not the anchor). All 4 columns / 8 runs: VERIFIED·0.95, warnings `[]`, writer enumeration correct or honestly-counted (W167). The two apparent prose omissions were chased down and **neither is anchor-caused**: `TLX_OPS_ADJ_MISDATE` (stable 3/3) traced to the payload's own pass-through framing — see W175 below; `FSI_OPTIMIZER_PROCESSING_DATA_POP` (1 of 3 runs) was LLM sampling variance, complete on re-runs.

**WHY NOT FIXED.** The naive fix — exclude colprov sentinels from the L5 `min()` (the W173 `_is_colprov_injected_sentinel` discriminator is mechanically reusable there) — is **actively worse**: `min()` then falls to the genuine cosine top-1, which on colprov queries is semantic noise — observed live: `ABL_LEV_RATIO` (a NON-writer of N_ANNUAL_GROSS_INCOME) and `MKT_RISK_POSITION_MAPPING_POPULATION` (a NON-writer of N_ALLOCATION_RANK) would have won. The anchor block and the W83B calendar-check target would then name a non-writer instead of a real writer. A correct fix needs a colprov-specific anchor source (e.g. from `column_provenance["writers"]` with an honest `source` label) **plus an explicit W150-gate decision**: `detect_near_twin_ambiguity` fires only on `source == "semantic_top1"` ([anchor_resolution.py:355](src/agents/anchor_resolution.py#L355)), so the sentinel anchor is what holds the W150 gate open on colprov — re-labeling the source would silently delete the W173-pinned genuine-tie-still-hedges behavior (`test_w173_genuine_near_twin_tie_still_hedges_on_colprov_query`). Zero observed answer-level harm does not buy that design work.

**Three specifics to keep findable:**
1. **The mislabeled diagnostic** — `done.diagnostic.w70_anchor` + the `apply_w70_anchor … (source=semantic_top1, confidence=low)` log line are, on colprov queries, systematically a deterministic injection artifact wearing an embedding label. Benchmark/telemetry consumers of `w70_anchor` on the colprov path must not read it as a similarity ranking.
2. **The tie-break artifact** — when several writers are injected, all tie at 0.0 and `min()` returns the FIRST in `multi_source` iteration order (post-W89 manifest reorder), NOT the writer-set/resolver order. Observed: N_ALLOCATION_RANK anchored on `T2T_SUB_EXPOSURES_FCT_SUB_EXPOSURES` (iteration position 1), not the resolver's first writer. Which writer the diagnostic names is an ordering accident.
3. **The W150-gate dependency** — the sentinel anchor's `semantic_top1` source is the only thing keeping the near-twin hedge gate OPEN on colprov queries (margin now computed over genuine candidates only, per W173). Any future L5/anchor-source change on this path changes W150 reachability and must decide that trade-off explicitly, not inherit it.

**Sibling site (recorded, not a consumer).** `hierarchy_header` ([logic_explainer.py:3578-3581](src/agents/logic_explainer.py#L3578-L3581)) independently re-derives "top-ranked multi_source entry by score" — the same min-score-lands-on-sentinel logic in a second place. Masked in all probes by W81 cross-process suppression; a single-process colprov answer would render the sentinel writer's hierarchy (still a real writer). An L5-only change would NOT touch this site.

**Revisit conditions.** Re-open only if: (a) a colprov query is observed where the L5 anchor lands on a NON-writer or demonstrably changes the user-facing answer; (b) something starts consuming `w70_anchor` as a similarity signal on the colprov path (e.g. benchmark scoring, a new W57 check); or (c) the W150 gate condition / anchor-source taxonomy is being redesigned anyway — fold the colprov anchor source into that work.

---

## W175. "What writes X?" omits pass-through writers — payload frames them as not-transformed — WATCH ITEM (W159/W167 class; needs a product decision)

Surfaced during the W174 diagnostic (live @ `e144f9f`), **distinct from L5/W174 — anchor-independent.** `What writes N_ANNUAL_GROSS_INCOME?` stably (3/3 runs) answers with only `FN_LOAD_OPS_RISK_DATA` at VERIFIED·0.95, omitting the second attested writer `TLX_OPS_ADJ_MISDATE` (INSERT, confirmed by `resolve_column_writers`). The omitted writer WAS in the prompt: the deterministic payload rebuild (byte-identical 2567 chars to the live run) shows `assemble_llm_payload` rendered it as `--- STEP 4 [PASS-THROUGH]: TLX_OPS_ADJ_MISDATE --- … The value is not transformed -- this function date-adjusts historical records.` — the payload itself tells the LLM the writer doesn't transform the value, and the prose then drops it from a "what writes" enumeration. No gate catches this: W169's scope-drift check fires on out-of-scope attribution, not omission; W167's completeness notice fires only on budget truncation (this payload is under budget). The badge is honestly VERIFIED because nothing stated is wrong — the answer is silently incomplete.

Not opened as active work because it needs a product decision first: **is enumeration-completeness a contract for "What writes X?"** — i.e., must pass-through/date-adjust writers be named alongside transforming writers? If yes, the fix surface is `assemble_llm_payload`'s pass-through framing (or a deterministic writer-list line in the payload, W167-style), NOT the anchor path. If no, this is working as intended and the entry is the record. Note the same pass-through de-emphasis is load-bearing elsewhere (it is what keeps trace narratives focused on transforming steps), so any change must check the W151/W159 fan-in surfaces. Decide before building.

---

## W55. Classify model-swap gate — EMPIRICALLY VALIDATED 2026-06-11 (evidence record, not a new defect; the gate stays)

**Status:** EVIDENCE-FOR-GATE. W55 has until now existed only as the comment in [src/llm_factory.py:59-61](src/llm_factory.py#L59-L61) ("Remaining sites (orchestrator.classify_query) are intentionally absent — see W55") recording the W34c Phase 2 revert. This entry is the live empirical case for it, so the "why not just use the cheaper model for classify?" question has an on-record answer with evidence instead of being re-derived every time latency comes up. Provenance: classify-model divergence diagnostic (2026-06-11), 10 live `/v1/stream` runs against main @ `c5b8221` — classify surface byte-identical at `95113f2` and `fcc10c4` (intervening merges are fixture-only / query_engine-only). Trigger: the UI model dropdown, which swaps **ALL** LLM sites including classify ([client.js:70](frontend/src/api/client.js#L70) `body.model` → `QueryRequest.model` → [main.py:1061-1063](src/main.py#L1061-L1063) `classify_query(..., model=model)` → `create_llm`, where an explicit `model` always beats the W34c per-site pins). The dropdown is therefore a live preview of the W55 swap.

**Finding (deterministic 3/3 per model).** Query: `which are the tables involved in g test function` — vague phrasing, no anchorable token (`g test`: no underscore, <6 chars, so `extract_function_candidates` can't catch it and the route rides entirely on the classify LLM).
- **gpt-4o-mini** → `query_type: UNSUPPORTED`, `confidence: 0.0`, **fabricated** `unsupported_reason: "function table dependencies cannot be traced"` (2 runs; "…are not introspectable" on the third — route deterministic, wording flaky). Result: DECLINED in ~4 s, no retrieval, rendered with `validated: true`. **The decline is FALSE** — ground truth from [FN_G_TEST_CSTM.sql](../db/modules/ABL_CAR_CSTM_V4/functions/FN_G_TEST_CSTM.sql) confirms the table dependencies ARE in the parsed source (9 write targets, ~17 read tables).
- **gpt-5-mini** → `query_type: COLUMN_LOGIC`, confidence 0.70/0.85/0.75 → the pipeline produced a line-accurate table list: every cited table verified present in the FN_G_TEST_CSTM source at the cited lines (e.g. DIM_BASEL_ASSET_CLASS 76-121, REVELEUS_PARAMETER_MASTER 156-197, MERGE blocks 505-598/742-841/920-963), **zero fabricated tables** despite 35-function-wide retrieval (W108-truncated to 30). Nuance, so this isn't read as "5-mini perfect": badge honestly UNVERIFIED·0.4, and completeness varied run-to-run (one run omitted FSI_PARTY_RATING_DETAILS and ABL_ERROR_LOG; one run's markdown truncated mid-list). What it asserted was grounded; what it omitted varied.

**Two independent failure modes confirm the gate:**
1. **MISROUTING (new evidence).** gpt-4o-mini routes an answerable query to UNSUPPORTED with an invented capability claim — a silent trust failure: the DECLINED template renders `validated: true` (see W176 below).
2. **SCHEMA-BREAK (live repro of the documented revert cause).** On a genuinely-UNSUPPORTED sibling (the C14 reconciliation query), gpt-4o-mini returned only `{"query_type": "UNSUPPORTED", "unsupported_reason": …}` — 4 required `ClassificationResult` fields missing → pydantic `ValidationError` → the SSE stream died with a raw `error` event. This is exactly the failure that reverted classify from W34c Phase 2 ([llm_factory.py:38-42](src/llm_factory.py#L38-L42)). So 4o-mini fails both ways: misroutes answerable queries AND breaks the contract on correctly-declined ones.

**Key nuance: the divergence is PHRASING-dependent, not capability-dependent.** The sibling `which are the tables involved in FN_G_TEST_CSTM function` (explicit name) routes COLUMN_LOGIC on **both** models (4o-mini confidence 0.7, badge VERIFIED). The cheaper model fails precisely at the ambiguous cases — where the deterministic anchor layers can't help — while looking fine on easy ones. Spot-checks on named queries will NOT surface this.

**Implication for the W34 latency lever (the path, not a closed door).** The classify latency win (~3.5 s vs ~8 s per classify, observed) is available ONLY after: (a) the W55 structured-output hardening (ClassificationResult schema stability on UNSUPPORTED routes), AND (b) a Tier-1 routing canary of this shape — a "vague function reference + capability question" case. Tier 1 has NO such canary today (C14/C15 cover reconciliation/forecasting declines; nothing pins a vague-but-answerable route), which is why this failure would have slipped through the existing gate. Sequence: W55 hardening → routing canary → swap. Cross-refs: W34c thread (llm_factory comments), C14 canary (`protects: "… W34c #1"`), W165/W166 (same `g test` query family — those cover the W87 abstain path on described names; this covers the classify route upstream of it).

---

## W176. Decline template stamps `validated: true` on an LLM-authored reason — WATCH ITEM (trust-path defect, fixable; record only for now)

`_unsupported_stream` ([main.py:3591-3626](src/main.py#L3591-L3626)) interpolates the classifier's unverified one-liner (`unsupported_reason`) into a canned "scoped to the parsed PL/SQL graph / FCT_*" template and emits it as an authoritative capability statement — `done` payload: `"validated": True, "badge": "DECLINED"`. Nothing on the decline path checks the reason against anything; the boilerplate asserts specific capability limits ("not in the graph", "does not reconcile against FCT_*") that were never looked up. In the 4o-mini false-decline above, a fabricated capability denial rendered as a validated authoritative claim. Independent of W55 itself — the current gpt-5-mini classify doesn't fabricate these (C14/C15 declines are genuine) — but the PATH has no guard, so any future classify regression (or model swap) inherits instant authoritative-looking fabrication. Watch-item for a future trust-path pass: don't stamp `validated: true` on an LLM-authored reason, and/or make the boilerplate stop asserting unchecked specifics. No fix now.
