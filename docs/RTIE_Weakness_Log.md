...
- **Next investigation — W80a-impact audit (cheap pre-W80c attempt):** Pull verbatim descriptions of the 3 missing targets (SIGNIFICANT_INVESTMENT_IN_PARTY_FOR_REPORTING_BANK_IDENTIFICATION, SIGNIFICANT_INVST_THRESHOLD_TREATMENT_DATA_POP, SIGNFCNT_INVSTMNT_CAP_DEDUCTION_EXPOSURES) from `rtie:vec:OFSERM:<fn>`. Compare vocabulary against the canary query and against the 2 surfacing targets. If the missing targets' descriptions lack "non-regulated entity" framing or related semantic content prominently, targeted description regeneration (W80a-scoped) becomes a cheap pre-W80c attempt. Re-run the canary post-regeneration; if recall lifts to >=3/5, W80c stays deferred. If it stays at 2/5 with regenerated descriptions, W80c (hybrid graph + vector with rerank, gated on [W88](#w88-named-regulatory-computation-pre-router--diagnostic-in-flight-2026-05-12)) is justified with sharper evidence.
- **Long-term destination:** W80c — hybrid graph + vector retrieval with rerank — remains the architecturally correct fix. Gated on the W80a-impact audit (above) to confirm the description-quality lever has been exhausted, and on W36 Phase 7 / [W88](#w88-named-regulatory-computation-pre-router--diagnostic-in-flight-2026-05-12) preconditions for graph-aware retrieval.
- **Merge SHA:** pending.
...
- **W80 family structure:** This entry is the original umbrella — cross-table multi-stage VARIABLE_TRACE retrieval via graph-edge traversal — still open. Slices that have shipped or are tracked separately:
  ...
  - W80c (and later) — additional slices logged as they're scoped.
