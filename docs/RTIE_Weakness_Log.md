...
## W169. Framing-drift — anchor named as framing, substantive subject a different function — OPEN

**Status:** OPEN (facts-only stub; diagnose-first, no design yet). Surfaced during W168 live validation as the residual W168 is structurally blind to.
...
**Diagnose-first hook.** W57 Check 3a `_w57_check_anchoring` ([src/agents/logic_explainer.py](src/agents/logic_explainer.py)) ALREADY does dominance (asked-vs-prose, 2× frequency ratio, gated on ≥2 analyzed functions) and ALSO failed to flag Q12 live. Understand WHY it missed (likely: the framing mention keeps the anchor's body count non-zero so the 2× ratio isn't exceeded; and/or the asked-extraction path) BEFORE designing a new dominance check — the answer determines whether W169 is a Check-3a strengthening, a new gate, or an upstream retrieval / Phase-2 problem (the fabricated sub-function walkthrough smells upstream).
